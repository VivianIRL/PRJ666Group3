...
const INITIAL_POSTS = [
  {
    ...
    replies: [
      { id: 1, author: 'Ahmed M.', initials: 'AM', time: '1 hour ago', text: 'Mine took about 6 weeks. Make sure you check your IRCC account for any missing document requests.' },
      { id: 2, author: 'Sofia L.', initials: 'SL', time: '45 min ago', text: 'Processing times vary a lot right now. I\'d recommend calling the IRCC call centre if it\'s been more than 8 weeks.' },
    ],
    replyCount: 12,
    views: 34,
  },
  {
    ...
    replies: [
      { id: 1, author: 'Priya R.', initials: 'PR', time: '3 hours ago', text: 'Also Scotiabank has a great newcomer package. I opened mine within a week of arriving!' },
    ],
    replyCount: 8,
    views: 61,
  },
...
export default function Community() {
  ...
  function submitReply(postId) {
    ...
    setPosts(prev => prev.map(p => {
      ...
      return {
        ...p,
        replies: [...p.replies, { id: Date.now(), author: 'Rasa', initials: 'R', time: 'Just now', text }],
  ...
  const filteredPosts = posts.filter(p => {
    ...
    return !q || p.title.toLowerCase().includes(q) || p.tags.join(' ').toLowerCase().includes(q) || p.body.toLowerCase().includes(q);
  });
  ...
  return (
    <div className="community-page">
      <Container fluid className="community-container">
        ...
        <p className="community-subtitle">Ask questions, share tips, and find support from newcomers</p>
          ...
          <Col md={8}>
            ...
            <div className="create-post-card mb-3">
              ...
              <Form.Control
                ...
                value={postText}
                onChange={e => { setPostText(e.target.value); if (postError) setPostError(''); }}
                className="post-textarea"
              />
              <div className="post-controls mt-2">
                <Form.Select value={postType} onChange={e => setPostType(e.target.value)} className="post-type-select">
              ...
              <div className="tag-pills mt-2">
                {['Legal', 'Financial', 'Academic', 'Housing', 'Healthcare', 'Jobs'].map(tag => (
                  <button
                    key={tag}
              ...
              <div className="post-bottom mt-3">
                ...
                <Button className="btn-post ms-auto" onClick={handlePost}>Post</Button>
              </div>
            </div>

            {/* Posts */}
            {filteredPosts.length === 0 && (
              <div className="text-center text-muted py-5">No posts match your search.</div>
            )}
            {filteredPosts.map(post => (
              <div key={post.id} className="post-card mb-3">
                <div className="post-title" onClick={() => toggleExpand(post.id)}>{post.title}</div>
                <div className="post-meta"><strong>{post.author}</strong> · {post.time}</div>
                <div className="post-tags">
                  {post.tags.map(tag => (
                    <span key={tag} className={`post-tag ${TAG_COLORS[tag] || 'tag-general'}`}>{tag}</span>
                  ))}
                </div>
                <div className="post-body">{post.body}</div>
                <div className="post-footer">
                  <span className="post-stats">{post.replyCount} replies · {post.views} views</span>
                  <div className="post-actions">
                    <button className="action-btn" onClick={() => toggleExpand(post.id)}>
                ...
                {expandedPosts[post.id] && (
                  <div className="replies-section">
                    {post.replies.map(r => (
                      <div key={r.id} className="reply-item">
                        ...
                        <div className="reply-bubble">
                          <div className="reply-author">{r.author} · {r.time}</div>
                    ...
                    <div className="reply-input-row">
                      ...
                      />
                      <Button className="btn-reply" onClick={() => submitReply(post.id)}>Reply</Button>
                    </div>
                  </div>
                )}
              </div>
